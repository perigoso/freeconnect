...
module body(columns, body_height, plug_height, spacing, dowel_heigth, dowel_diameter, dowel_y_spacing, dowel_pertrusion, pogo_height, pogo_diameter, pogo_pertusion){
        difference()
        {
            ...
            pogo_array(columns, spacing, pogo_height, pogo_diameter, pogo_pertusion);
            dowels(dowel_heigth, dowel_diameter, columns, spacing, dowel_y_spacing, dowel_pertrusion);
            pcb(columns, pogo_diameter, 11, spacing);
        }
    }
    
module body_cover(columns, body_height, plug_height, spacing){
        difference()
        {
            translate([(columns-1)*spacing/2, spacing/2, 0])
                roundedRect([(columns+1)*spacing + 2, spacing + 2, body_height], 0.5);
...
module pcb(c, d, height, spacing){
    translate([-d/2-0.1, d/2, height-1])
        cube([(c-1)*spacing + d + 0.2, 0.6, 10]);
}
